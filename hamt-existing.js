...
/**
    Is `node` a leaf node?
*/
const isLeaf = (node) => node.type === LEAF || node.type === COLLISION;
...
const mergeLeaves = (shift, h1, n1, h2, n2) => {
  ...
  return IndexedNode(
    ...
    subH1 === subH2 ? [mergeLeaves(shift + SIZE, h1, n1, h2, n2)] : subH1 < subH2 ? [n1, n2] : [n2, n1],
  );
};
...
const IndexedNode__modify = function (shift, op, h, k, size) {
  ...
  if (!exists) {
    ...
    return children.length >= MAX_INDEX_NODE
      ...
      : IndexedNode(mask | bit, arraySpliceIn(indx, newChild, children));
  }
...
const ArrayNode__modify = function (shift, op, h, k, size) {
  ...
  if (child && !newChild) {
    ...
    return count - 1 <= MIN_ARRAY_NODE
      ...
      : ArrayNode(count - 1, arrayUpdate(frag, undefined, children));
  }
...
/**
    ...
    @see `tryGetHash`
*/
const tryGet = (hamt.tryGet = (alt, key, map) => tryGetHash(alt, hash(key), key, map));
...
*/
const getHash = (hamt.getHash = (hash, key, map) => tryGetHash(undefined, hash, key, map));
...
/**
    ...
    @see `get`
*/
const get = (hamt.get = (key, map) => tryGetHash(undefined, hash(key), key, map));
...
*/
const hasHash = (hamt.has = (hash, key, map) => tryGetHash(nothing, hash, key, map) !== nothing);
...
*/
const has = (hamt.has = (key, map) => hasHash(hash(key), key, map));
...
/**
    Is `value` a map?
*/
hamt.isMap = (value) => !!(value && value.__hamt_isMap);

/**
    Does `map` contain any elements?
*/
hamt.isEmpty = (map) => hamt.isMap(map) && !map.root;
...
/**
    ...
    @see `modifyHash`
*/
const modify = (hamt.modify = (f, key, map) => modifyHash(f, hash(key), key, map));
...
/**
    ...
    @see `setHash`
*/
const set = (hamt.set = (key, value, map) => setHash(hash(key), key, value, map));
...
const removeHash = (hamt.removeHash = (hash, key, map) => modifyHash(del, hash, key, map));
...
/**
    ...
    @see `removeHash`
*/
const remove = (hamt.remove = (key, map) => removeHash(hash(key), key, map));
...
*/
const appk = (k) => k && lazyVisitChildren(k.len, k.children, k.i, k.f, k.k);
...
*/
const visit = (map, f) => new MapIterator(map.root ? lazyVisit(map.root, f) : undefined);
...
const entries = (hamt.entries = (map) => visit(map, buildPairs));
...
const keys = (hamt.keys = (map) => visit(map, buildKeys));
...
const values = (hamt.values = Map.prototype.values = (map) => visit(map, buildValues));
...
/**
    ...
    @param map HAMT
*/
const forEach = (hamt.forEach = (f, map) => fold((_, value, key) => f(value, key, map), null, map));
